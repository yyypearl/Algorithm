...
## 📝 정리하기
- **[⭐] 각 자리수 더하는 경우**
1) 숫자인 n을 string으로 바꾸어 준다 ```String(n)```
2) 문자열에 쓰이는 메서드인 split을 사용해 각 자릿수 별로 쪼개 배열을 만들어 준다. ```.split('')```
3) 배열에 사용하는 메서드인 **reduce**를 사용해 각 자릿수의 합을 구한다. ```.reduce((acc, cur) => acc + cur * 1, 0);```
4) 현재 reduce 메서드의 cur값은 문자열이기 때문에 숫자 처리를 하기 위해 다음과 같은 방법을 처리해줄 수 있다. <br/>
