...
**- [🎁] 접근법**
- end 좌표를 기준으로 오름차순 정렬하여, ```이전 end 포인트 <= 현재 start 포인트```일 경우에만 구분 되었다고 생각하고 ```count``` 늘리는 방식!
- 만약, ```이전 end 포인트 > 현재 start 포인트``` 라면 겹치는 부분이 존재하기에 count를 늘릴 필요가 없다. <br/>
  (```이전 end 포인트```는 구분 되어졌을 때만 업데이트)

=> 문제에서 제시된 그림 말고 다른 방식으로도 접근해보는 사고를 가져보자!
